...
class TestDynamoDBPartiQLBackend(unittest.TestCase):
    ...
    def _get_base_config(self, table_name="test_table", **overrides):
        """Create a base configuration dictionary with defaults."""
        config = {
    ...
    def test_check_query_configuration_sort_with_base_table_hash_key_equality(self, mock_logger_arg):
        ...
        processed_config = self.backend._check_query_configuration(config, self.mock_model)
    ...
    def test_records_simple_fetch(self, mock_logger_arg):
        ...
        self.mock_dynamodb_client.execute_statement.assert_called_once_with(**expected_call_kwargs)
        self.assertEqual(len(results), 2)
        # Assert based on what _map_from_boto3 currently does
        self.assertEqual(results[0], {"id": "user1", "name": "Alice", "age": Decimal("30")})
    ...
    def test_records_with_limit(self, mock_logger_arg):
        ...
        expected_client_token = self.backend.serialize_next_token_for_response(ddb_next_token)
        self.assertEqual(
    ...
    def test_records_pagination_flow(self, mock_logger_arg):
        ...
        client_sends_this_token = self.backend.serialize_next_token_for_response(initial_ddb_token)
        ...
        next_page_data = {}
        results_page1 = list(self.backend.records(config1, self.mock_model, next_page_data))

        expected_call_kwargs1 = {
            "Statement": expected_statement,
            "NextToken": initial_ddb_token,
        }
        self.mock_dynamodb_client.execute_statement.assert_called_once_with(**expected_call_kwargs1)
        ...
        self.mock_dynamodb_client.execute_statement.return_value = {"Items": ddb_items_page2}
        next_page_data = {}
        results_page2 = list(self.backend.records(config2, self.mock_model, next_page_data))
        expected_call_kwargs2 = {
            "Statement": expected_statement,
            "NextToken": ddb_next_token_page1,
        }
        self.mock_dynamodb_client.execute_statement.assert_called_once_with(**expected_call_kwargs2)
    ...
    def test_records_limit_cuts_off_ddb_page(self, mock_logger_arg):
        ...
        expected_client_token = self.backend.serialize_next_token_for_response(ddb_next_token_after_limit)
        self.assertEqual(
            next_page_data["next_token"],
            expected_client_token,
        )
    ...
    def test_create_record(self, mock_logger_arg):
        ...
        expected_statement = "INSERT INTO \"my_test_table\" VALUE {'id': ?, 'name': ?, 'age': ?}"
    ...
    def test_update_record(self, mock_logger_arg):
        ...
        self.mock_dynamodb_client.execute_statement.return_value = {"Items": [updated_item_from_db]}

        updated_data_response = self.backend.update(record_id, update_data, self.mock_model)
